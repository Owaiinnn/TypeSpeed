...
// ═══════════════════════════════════════════════════════════════════
// API FETCHER — pulls random code snippets from the internet
// Falls back to built-in snippets if offline or API fails
// ═══════════════════════════════════════════════════════════════════

const API_TIMEOUT = 4000; // ms

/**
 * Fetches a random code snippet from a public API.
 * Uses GitHub's search to find small, clean code snippets.
 * Returns null if the API is unreachable.
 */
export async function fetchSnippetFromAPI(
  difficulty?: CodeSnippet["difficulty"]
): Promise<CodeSnippet | null> {
  // Use a set of common search terms to get varied results
  const searchTerms = [
    "function", "class", "async", "const", "import",
    "interface", "export", "return", "promise", "array",
    "map", "filter", "reduce", "useState", "useEffect",
    "struct", "impl", "enum", "match", "trait",
    "lambda", "decorator", "yield", "comprehension", "dataclass",
  ];
  const term = searchTerms[Math.floor(Math.random() * searchTerms.length)];
  const languages = ["javascript", "typescript", "python", "rust", "go"];
  const lang = languages[Math.floor(Math.random() * languages.length)];

  const maxLines = difficulty === "easy" ? 6 : difficulty === "hard" ? 20 : 12;
  const minLines = difficulty === "easy" ? 2 : difficulty === "hard" ? 8 : 4;

  try {
    const url = `https://api.github.com/search/code?q=${term}+language:${lang}+size:50..500&per_page=10&page=${Math.floor(Math.random() * 5) + 1}`;
    ...
    // Pick a random result and fetch its raw content
    const item = parsed.items[Math.floor(Math.random() * parsed.items.length)];
    const rawUrl = item.git_url;

    if (!rawUrl) { return null; }

    const blobData = await httpGet(rawUrl);
    ...
    // Extract a clean snippet from the file
    const snippet = extractCleanSnippet(content, minLines, maxLines);
...
function extractCleanSnippet(
  ...
  // Find a good starting point — look for function/class definitions
  const starters = [
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (starters.some((r) => r.test(trimmed))) {
      startIdx = i;
      break;
    }
  }
  ...
  // Take a slice of reasonable length
  const slice = lines.slice(startIdx, startIdx + maxLines);
  if (slice.length < minLines) { return null; }

  const result = slice.join("\n").trimEnd();

  // Validate: no super long lines, no binary junk, no empty
  if (!result || result.length < 20 || result.length > 800) { return null; }
  if (result.split("\n").some((l) => l.length > 80)) { return null; }
...
function httpGet(url: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const req = https.get(
      ...
      {
        headers: {
          "User-Agent": "TypeSpeed-VSCode/0.1",
...
// ═══════════════════════════════════════════════════════════════════
// BUILT-IN SNIPPETS — 100+ code snippets, used as fallback
// ═══════════════════════════════════════════════════════════════════
...
export function getRandomSnippet(difficulty?: CodeSnippet["difficulty"]): CodeSnippet {
  ...
  return { ...snippet, source: "built-in" };
}
